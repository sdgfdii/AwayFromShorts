...
2. **屏蔽计划** — 勾选星期,添加/删除时间段;同页的「🔒 强制模式」: 开启后**仅在屏蔽时段内**强制——时段内无法关闭/解除,且**当天(至 24:00)无法提前关闭**(防破戒设计,面板不提供关闭按钮、接口也拒绝),到 24:00 **自动解除、不会自动续到下一天**;时段外不强制、完全不影响使用
...
7. **统计** — ① 破戒统计:每次「临时解除」的次数与时长,四种视图一键切换:🗡 战报 / 🔥 热力图 / ▤ 控制台 / ⏱ 时间去向(ActivityWatch 式前台应用+站点时长,面板开启时自动记录,离开 3 分钟不计),可一键清空破戒记录
8. **关于/卸载** — 查看原理,一键卸载
