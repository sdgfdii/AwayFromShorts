...
1. **状态** — 查看当前是否在屏蔽中、hosts 状态、计划任务状态;一键"立即应用/临时屏蔽/临时解除";同页 **屏蔽计划**: 勾选星期,添加/删除时间段;「🔒 强制模式」: 开启后**仅在屏蔽时段内**强制——时段内无法关闭/解除,且**当天(至 24:00)无法提前关闭**(防破戒设计,面板不提供关闭按钮、接口也拒绝),到 24:00 **自动解除、不会自动续到下一天**;时段外不强制、完全不影响使用
