...
| 🔒 强制模式 | 开启后屏蔽时段内无法关闭/解除(管不住自己时用),随时可手动关闭;时段外不强制,正常使用 |
...
2. **屏蔽计划** — 勾选星期,添加/删除时间段;同页的「🔒 强制模式」: 开启后**仅在屏蔽时段内**强制(时段内无法关闭/解除、手改配置也会被 1 分钟内恢复),时段外不强制、完全不影响使用;强制模式**随时可以手动关闭**(生效中也一样)
...
- **强制模式**: 开启后仅在屏蔽计划时段内生效——时段内无法关闭屏蔽或临时解除(计划任务每分钟兜底校验,手改 `config.json` 删 force 也会在 1 分钟内被恢复);时段外不强制,正常使用。强制模式**随时可以手动关闭**(面板「关闭强制模式」或改 `config.json` 的 `force.enabled: false`),关闭后立即恢复普通屏蔽,计划任务不再兜底补开。
